...
def review_context(user, *, plan=None, product=None):
    """ Shared context for the reviews section on a plan/product detail page.
        Supplies the average rating, count, the visitor's own review (if any), a
        prefilled form, and the write gate — used by both PlanDetailView and
        ProductDetailView.
    """
    ...
    user_review = reviews.filter(user=user).first() if user.is_authenticated else None
